...
Backend detect_backend() {
    ...
    (void)printf("[INFO] 加速方案: macOS（请求 GPU 路径，CoreML 由运行时决定）\n");
    return Backend::CoreML;

#elif defined(__linux__)
    // Linux: 优先 CUDA，回退 CPU
    (void)printf("[INFO] 检测加速方案: Linux — ");
    (void)fflush(stdout);
    ...
    if (fp) {
        ...
        if (fgets(buf, sizeof(buf), fp) != nullptr && buf[0] != '\0') {
            (void)pclose(fp);
            (void)printf("检测到 CUDA\n");
            return Backend::CUDA;
        }
        (void)pclose(fp);
    }
    (void)printf("未找到 NVIDIA 驱动，回退到 CPU 模式\n");
    return Backend::CPU;

#elif defined(_WIN32)
    // Windows: 优先 CUDA，回退 CPU
    (void)printf("[INFO] 检测加速方案: Windows — ");
    (void)fflush(stdout);
